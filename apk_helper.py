        ...
        if not isinstance(config_data["row_separator"], str):
            ...
            for char in sep:
                ...
                if code < 32 or code > 126:
                    errors.append("行间分隔符包含非可打印ASCII字符")
                    break
    ...
    if "col_separator" in config_data:
        if not isinstance(config_data["col_separator"], str):
            ...
            for char in sep:
                code = ord(char)
                if code < 32 or code > 126:
                    errors.append("列间隔符包含非可打印ASCII字符")
                    break
        ...
        if os.path.isfile(path):
            try:
                ...
                if valid:
                    config = config_data
                    loaded_path = path
                    app_logger.debug(f"成功加载配置文件: {path}")
                    break
                else:
...
def save_config():
    ...
    valid, errors = validate_config(config_data)
    if not valid:
        app_logger.error(f"配置数据验证失败，无法保存: {errors}")
        return False
    ...
    if appdata_dir:
        try:
            ...
            config = config_data
            app_logger.debug(f"配置已保存到: {appdata_path}")
            saved = True
        except Exception as e:
    ...
    if not saved and program_path:
        try:
            with open(program_path, 'w', encoding='utf-8') as f:
                json.dump(config_data, f, ensure_ascii=False, indent=2)
            config = config_data
            app_logger.debug(f"配置已保存到: {program_path}")
            saved = True
        except Exception as e:
            app_logger.error(f"保存配置到程序目录失败: {e}")
...
示例:
  ...
  %(prog)s -l DEBUG app.apk           # 设置日志级别为DEBUG并启动GUI
    ...
    if args.log_level:
        ...
        app_logger.debug(f"命令行指定日志级别: {log_level}")
